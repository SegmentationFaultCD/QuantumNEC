 ...
 */
auto strcmp( const char *FirstPart,
 ...
 */
auto strlen( const char *String ) -> std::uint64_t {
    auto strSize { 0ULL };
    auto tmp { String };
    while ( *tmp ) {
        ++tmp;
 ...
 * @return 开始dest指向的地址
 */
auto strchr( const char *dest, std::int32_t ch ) -> char * {
    if ( !dest ) {
        return nullptr;
    }
 ...
 * @return 开始dest指向的地址
 */
auto strrchr( const char *dest, std::int32_t ch ) -> char * {
    auto  _src_one { Library::strchr( dest, ch ) };
    char *_src_two { };
    while ( _src_one ) {
        _src_two = Library::strchr( _src_one + 1, ch );
        if ( _src_two ) {
            _src_one = _src_two;
        }
        else
...
auto strstr( char       *haystack,
    ...
    if ( *needle ) {
        while ( *haystack ) {
            for ( std::int32_t n { }; *( haystack + n ) == *( needle + n );
                  n++ ) {
                if ( !*( needle + n + 1 ) )
                    return haystack;
            }
...
auto memchr( void       *buf,
             std::int8_t value, std::uint64_t size )
    -> void * {
    if ( !buf )
        return nullptr;
...
auto memset( void         *dest,
    ...
    // 处理未对齐的字节
    while ( count > 0 && ( (std::size_t)ptr & ( sizeof( std::size_t ) - 1 ) ) ) {
        *ptr++ = val;
        count--;
    }

    // 如果还有足够的字节要处理
    if ( count >= sizeof( std::size_t ) ) {
        std::size_t  val_word;
        std::size_t *word_ptr;
