...
PUBLIC namespace QuantumNEC::Kernel {
    Heap::Heap( VOID ) noexcept {
        auto zone = new ( physical_to_virtual( Memory::page->malloc( 1, MemoryPageType::PAGE_2M ) ) ) Zone { };
    ...
    auto Heap::malloc( IN size_t size )->VOID * {
        ...
        if ( size < PAGE_2M_SIZE ) {
            ...
            if ( zone->block_table.is_empty( ) ) {
                ...
                if ( !node ) {
                    ...
                    if ( reinterpret_cast< uint64_t >( block ) + size > reinterpret_cast< uint64_t >( zone->zone_end_address ) ) {
                        ...
                        zone->is_full = TRUE;
                        // 并开辟新内存区域
                        zone = new ( physical_to_virtual( Memory::page->malloc( 1, MemoryPageType::PAGE_2M ) ) ) Zone { };
                        zone->zone_node.container = reinterpret_cast< decltype( zone->zone_node.container ) >( zone );
                        zone->is_full = FALSE;
                        zone->size = PAGE_2M_SIZE;
                        zone->zone_start_address = reinterpret_cast< decltype( zone->zone_start_address ) >( reinterpret_cast< uint64_t >( zone ) + sizeof *zone );
                    ...
                    block->is_free = FALSE;
                    zone->block_count++;
                    memset( block->start_address, 0, size );
                    lock.release( );
            ...
            zone = new ( physical_to_virtual( Memory::page->malloc( page_count, MemoryPageType::PAGE_2M ) ) ) Zone { };
            zone->zone_node.container = reinterpret_cast< decltype( zone->zone_node.container ) >( zone );
    ...
    auto Heap::free( IN VOID * address )->VOID {
        ...
        // 清空这块内存并设置为空闲
        memset( block->start_address, 0, block->size );
        block->is_free = TRUE;
        zone->block_count--;
