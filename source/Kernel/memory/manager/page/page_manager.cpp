...
PUBLIC namespace QuantumNEC::Kernel {
    using namespace std;
    __page_manager::__page_manager( VOID ) noexcept {
        ...
        for ( auto i = 0ul; i < Architecture::__config.memory_map.entry_count; ++i ) {
            ...
            auto start { entry->base }, end { start + __page_allocater::__page_size< MemoryPageType::PAGE_2M > };
            if ( entry->type == LIMINE_MEMMAP_USABLE ) {
                if ( start / __page_allocater::__page_size< MemoryPageType::PAGE_2M > > 0 ) {
        ...
        bitmap->set( reinterpret_cast< uint64_t >( virtual_to_physical( info_address ) ) / __page_allocater::__page_size< MemoryPageType::PAGE_2M > );
        bitmap->set( 0, 32 );
        for ( auto i = 0ul; i < Architecture::__config.memory_map.entry_count; ++i ) {
            auto entry = memory_descriptor->entries[ i ];
            ...
            case LIMINE_MEMMAP_BOOTLOADER_RECLAIMABLE:     // 启动时服务可回收内存
                // 如果是这三种类型那么就计算空闲内存
                start = Lib::DIV_ROUND_UP( start, __page_allocater::__page_size< MemoryPageType::PAGE_2M > );
                end = end / __page_allocater::__page_size< MemoryPageType::PAGE_2M >;
                // 统计空闲内存
                if ( end >= start ) {
                    this->free_memory_total += ( end - start ) * __page_allocater::__page_size< MemoryPageType::PAGE_2M >;
                }
            ...
            case LIMINE_MEMMAP_FRAMEBUFFER:            // 显存占用的
                ...
                auto group_base_address = (uint64_t)start & ~( __page_allocater::__page_size< MemoryPageType::PAGE_2M > * PH::page_descriptor_count * PH::page_header_count - 1 );
                // 计算取得所在组的块的编号
