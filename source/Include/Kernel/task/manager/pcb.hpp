...
#error Not any registers
    ...
    public:
        ...
        explicit PCB( IN std::tuple< const char_t *, uint64_t, VOID *, State, std::pair< FpuEnable, FpuUsed >, Type > group, IN uint64_t _arg ) noexcept {
            ...
            lock.acquire( );
            // 内核栈处理
            this->kernel_stack_base = reinterpret_cast< uint64_t >( this ) + TASK_STACK_SIZE - __page_allocater::__page_size< MemoryPageType::PAGE_4K >;
            this->kernel_stack_size = __page_allocater::__page_size< MemoryPageType::PAGE_4K >;
            ...
            if ( _type == Type::USER_PROCESS ) {
                __page_table { }.__copy( this->memory_manager.page_table );     // 制作页表
                this->user_stack_base = (uint64_t)__page_allocater { }.__allocate< MemoryPageType::PAGE_2M >( 4 );
                this->user_stack_size = __page_allocater::__page_size< MemoryPageType::PAGE_2M > * 4;
                *(uint64_t *)physical_to_virtual( this->user_stack_base ) = uint64_t( this );
                __mapper { }.__map( this->user_stack_base,
...
#error Not any registers
#endif
            __page_table { }.__activate( this->memory_manager.page_table );
    ...
    inline auto get_current( VOID ) {
        ...
        rsp = (uint64_t)__page_table { }.__VTP_address_from( (VOID *)rsp, MemoryPageType::PAGE_2M, page_table );
