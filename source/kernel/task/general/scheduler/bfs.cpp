...
PUBLIC namespace QuantumNEC::Kernel {
    auto BrainFuckScheduler::__pick_next__( VOID ) -> std::expected< PCB *, ErrorCode > {
        ...
        if ( index < 100 ) {
            // 实时任务，直接弹出首位
            auto pcb = (PCB *)SchedulerHelper::task_queue[ index ].begin( )->container;
            ...
            for ( auto i = index; i < SchedulerHelper::total_priority; ++i ) {
                ...
                auto head = SchedulerHelper::task_queue[ i ].begin( );
    ...
    auto BrainFuckScheduler::__wake_up__( PCB * pcb ) -> PCB * {
        ...
        auto node = SchedulerHelper::running_queue.traversal(
        ...
        if ( !node ) {
            ...
            SchedulerHelper::running_queue.remove( can_seize_pcb->general_task_node );
            SchedulerHelper::task_queue[ can_seize_pcb->priority ].append( can_seize_pcb->general_task_node );
    ...
    auto BrainFuckScheduler::__schedule__( VOID ) -> std::expected< PCB *, ErrorCode > {
        auto pcb = (PCB *)SchedulerHelper::running_queue.traversal(
