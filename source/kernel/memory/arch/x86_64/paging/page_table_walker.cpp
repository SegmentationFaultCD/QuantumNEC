...
PUBLIC namespace QuantumNEC::Kernel::x86_64 {
    ...
    auto PageTableWalker::page_protect( IN BOOL flags ) -> VOID {
        lock.acquire( );
    ...
    auto PageTableWalker::activate( IN pmlxt & pmlxt ) -> VOID {
        lock.acquire( );
        if ( pmlxt.get_table( ) ) {
            CPU::set_page_table( (uint64_t *)virtual_to_physical( pmlxt.get_table( ) ) );
        }
        lock.release( );
    }
