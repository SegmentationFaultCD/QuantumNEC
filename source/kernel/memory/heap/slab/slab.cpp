...
PUBLIC namespace QuantumNEC::Kernel {
    SlabManager::SlabManager( VOID ) noexcept {
        ...
        std::memset( (void *)zone, 0, PageAllocater::__page_size< MemoryPageType::PAGE_2M > * 4 );
        for ( auto i = 0ul; i < this->cache_size_count; ++i ) {
            ...
            this->slab_caches[ i ].cache_pool->free_count   = PageAllocater::__page_size< MemoryPageType::PAGE_2M > / this->cache_size[ i ];
            this->slab_caches[ i ].cache_pool->color_length = ( ( PageAllocater::__page_size< MemoryPageType::PAGE_2M > / this->cache_size[ i ] + sizeof( uint64_t ) * 8 - 1 ) >> 6 ) << 3;
        ...
        for ( auto i = 0ul; i < this->cache_size_count; ++i ) {
            ...
            std::memset( this->slab_caches[ i ].cache_pool->virtual_address, 0, PageAllocater::__page_size< MemoryPageType::PAGE_2M > );
        }
    }
}