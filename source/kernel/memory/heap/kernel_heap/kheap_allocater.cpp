...
auto KHeapAllocater::allocate( uint64_t size ) -> void * {
    ...
    if ( result.has_value( ) ) {
        ...
        return slab_cache.visit( [ &, this ]( const Lib::shared_spinlock< SlabCache > &slab_cache ) -> void * {
            ...
            if ( slab_cache.value( ).total_free ) {
                ...
                if ( slab = [ &slab_cache, this ] -> Slab * {
                         ...
                         case 512: {
                             ...
                             auto struct_size      = sizeof( Slab ) + PageWalker::__page_size__< MemoryPageType::PAGE_2M > / slab_cache.value( ).size / 8;
                             ...
                             slab->free_count      = ( PageWalker::__page_size__< MemoryPageType::PAGE_2M > - ( PageWalker::__page_size__< MemoryPageType::PAGE_2M > / slab_cache.value( ).size / 8 ) - sizeof( Slab ) / slab_cache.value( ).size );
                             slab->using_count     = 0;
                         ...
                         case 1048576: {
                             ...
                             slab->free_count      = PageWalker::__page_size__< MemoryPageType::PAGE_2M > / slab_cache.value( ).size;
                             slab->using_count     = 0;
                             slab->color_count     = slab->free_count;
                             slab->color_length    = ( ( slab->color_count + sizeof( uint64_t ) * 8 - 1 ) >> 6 ) << 3;
            ...
            for ( auto i = 0ul; i < slab->color_count; ++i ) {
                ...
                if ( !( *( slab->color_map + ( i >> 6 ) ) & ( 1ul << ( i % 64 ) ) ) ) {
                    *( slab->color_map + ( i >> 6 ) ) |= 1ul << ( i % 64 );
                    ...
                    auto virtual_address = (void *)( (uint64_t)slab->virtual_address + slab_cache.value( ).size * i );
