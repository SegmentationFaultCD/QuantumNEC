...
PUBLIC namespace QuantumNEC::Kernel {
        ...
        if ( node ) {
            ...
            if ( end_remainder <= PH::__helper__::page_descriptor_count ) {
                ...
                for ( auto i = 0ul; i < zone->header_count; ++i ) {
                    if ( page_headers[ i ].flags.state != PHI::__page_flags__::__page_state__::ALL_FREE ) {
                        goto finish;
                    }
                }
            }
    ...
    auto PageCollector::free< MemoryPageType::PAGE_4K >( IN VOID * __physical_address__, IN uint64_t __size__ ) -> VOID {
        ...
        if ( node ) {
            ...
            // 头
            uint64_t head_free_remainder;
            if ( __size__ >= PH::__helper__::page_descriptor_count ) {
                head_free_remainder = PH::__helper__::page_descriptor_count - index;     // 头的要释放根数量
            }
            else {
                head_free_remainder = __size__;
            }
