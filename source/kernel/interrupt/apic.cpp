 ...
 */
    ...
    struct ApicInformation {
        ...
        uint64_t          core_count;
        uint64_t          local_apic_ID[ Memory::GDT::GDT_COUNT ];
...
public:
    static auto write_apic( uint16_t index, uint64_t value, ApicType type ) -> void;
    static auto read_apic( uint16_t index, ApicType type ) -> uint64_t;
    static auto enable_xapic( void ) -> void;
    static auto enable_x2apic( void ) -> void;
    static auto eoi( const irq_t irq ) -> void;
    /**
     * @brief 检测CPU是否支持APIC
     * @return 如果CPU支持且未在MSR中禁用，则返回 TRUE 值
     */
    static auto check_apic( void ) -> SupportState;
    static auto irq_set_mask( irq_t irq ) -> void;
    static auto irq_clear_mask( irq_t irq, uint8_t vector, uint8_t ) -> void;
    static auto enable_ioapic( irq_t irq ) -> void;
    static auto disable_ioapic( irq_t irq ) -> void;
    static auto install_ioapic( Apic::irq_t irq, void *entry ) -> uint64_t;
    static auto uninstall_ioapic( irq_t irq ) -> void;
    static auto ioapic_level_ack( irq_t irq ) -> void;
    static auto ioapic_edge_ack( irq_t irq ) -> void;
