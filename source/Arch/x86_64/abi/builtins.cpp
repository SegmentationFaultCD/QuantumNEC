...
PUBLIC namespace QuantumNEC::Architecture::ABI {
     ...
     *   s1
     */
    PUBLIC _C_LINK auto __builtin_memcpy( IN OUT VOID * s1, IN CONST VOID * s2,
                                          IN uint64_t n )
        ->Ptr< VOID > {
        auto dst { reinterpret_cast< uint8_t * >( s1 ) };
        auto src { reinterpret_cast< uint8_t * >( const_cast< VOID * >( s2 ) ) };
    ...
    PUBLIC _C_LINK auto __builtin_memcmp( IN CONST VOID * buf1,
                                          IN CONST VOID * buf2, IN uint64_t size )
        ->int32_t {
        uint64_t i { };
        auto p1 { reinterpret_cast< uint8_t * >( const_cast< VOID * >( buf1 ) ) };
        auto p2 { reinterpret_cast< uint8_t * >( const_cast< VOID * >( buf2 ) ) };
        while ( *p1++ == *p2++ ) { ++i; }
        if ( i != size )
            return -1;
        return 0;
    ...
    PUBLIC _C_LINK auto __builtin_memset( IN VOID * p, IN int32_t data,
                                          IN uint64_t size )
        ->VOID * {
        auto p1 { reinterpret_cast< uint8_t * >( p ) };
        for ( auto i { 0ul }; i < static_cast< uint64_t >( size ); ++i ) {
...
namespace std {
auto __throw_bad_function_call( ) -> void {
}
}     // namespace std