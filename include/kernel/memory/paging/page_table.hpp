...
//         }
//         auto operator[]( std::size_t index ) -> page_table_entry * {
//             if ( index >= 512 ) {
//                 return this->pmlx_table
//             }
//         }
...
class Paging {
    ...
    public:
        ...
        virtual auto operator=( std::tuple< uint64_t, uint64_t, uint64_t, Page::Type > group ) -> pmlxt & = 0;
        ...
        virtual auto set_base( uint64_t index, uint64_t address, Page::Type mode ) -> void = 0;
        virtual auto set_ps_pat( uint64_t index, bool bit ) -> void                        = 0;
    ...
    struct pml1t : pmlxt {
        ...
        virtual auto operator=( std::tuple< uint64_t, uint64_t, uint64_t, Page::Type > group ) -> pmlxt & override {
            auto &[ index, base, flags, _mode ] = group;
            this->set_p( index, !!( flags & PAGE_PRESENT ) );
            this->set_rw( index, !!( flags & PAGE_RW_W ) );
            this->set_us( index, !!( flags & PAGE_US_U ) );
        ...
        virtual auto flags_base( uint64_t index, Page::Type mode ) -> uint64_t override {
