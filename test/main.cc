...
//         requires std::invocable< OP, const T & > && std::same_as< bool, std::invoke_result_t< OP, const T & > >
...
//     //     rbtree.insert( *new RedBlackTree< int, int >::Node { i, new int { i } } );
//     // rbtree.traverse( []( const int &j ) -> bool { std::println( "{}", j ); return false; } );
//     // // list.traverse( []( const int &j ) -> bool { std::println( "{}", j ); return false; } );
// }